...
def plot_1d(path, ax=None, label=None, color="magenta", scale=None, xlim=(-120, -140)):
    ...
    # plot the spectrum
    ax.plot(uc.ppm_scale()+77.8189, data, color=color, label=label, linewidth=4)

    # decorate axes
    ax.set_yticklabels([])
    ax.set_yticks([])
    #ax.set_title("CTD $^{19}$F 1D")
    ax.set_xlabel("$^{19}$F (ppm)", fontsize=16, labelpad=10, fontweight="bold")
...
fig.savefig("figures/4F-for-thesis-intro.png", dpi=1200, transparent=True)
plt.show()